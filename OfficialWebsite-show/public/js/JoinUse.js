...
var commonalityFn = function() {
  return $(function() {
    ...
    for (let s = 0; s < sendbtnhover.length; s++) {
      sendbtnhover[s].addEventListener("mouseover", () => {
        imghover[s].src = "../public/img/join/社招-btn02.jpg";
      });
      sendbtnhover[s].addEventListener("mouseout", () => {
...
$.ajax({
  ...
  success: function(response) {
    ...
    $(".leftBtn").on("click", function() {
      ...
      $(".dotImge")[0].children[backImg].childNodes[0].src =
        "../public/img/join/dot-01.png";
      distance += 100;
    ...
    // 初始化轮播图分页器默认样式
    $(".dotImge")[0].children[0].childNodes[0].src =
      "../public/img/join/dot-02.png";
    //! 轮播图分页器点击事件
    $(".dotBtn").on("click", function() {
      distance = $(this).index() * 100;
      backImg = $(this).index();
      $(".marginDiv")[0].style.cssText = `margin-left:-${$(this).index() *
        100}%`;
      $(".whirligigBack")[0].style.cssText = `margin-left:-${$(this).index() *
        100}%`;
